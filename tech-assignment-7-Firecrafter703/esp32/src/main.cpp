...
void setupModel() {
    ...
    Serial.print("got model");
    static tflite::AllOpsResolver resolver;
    static tflite::MicroErrorReporter micro_error_reporter;
    static tflite::MicroInterpreter static_interpreter(
        model, resolver, tensor_arena, kTensorArenaSize, &micro_error_reporter);
    interpreter = &static_interpreter;
    Serial.print("before interpertor");
    interpreter->AllocateTensors();
    Serial.print("after interpor");
...
void mqttCallback(char* topic, uint8_t* payload, unsigned int length) {
    ...
    //dataRequested = true;
    if(mqttmessage == "get_one")
    {
        Serial.print("string is get_one, only send one time please");
        if(Freeze)
        {
        sendOne = true;
        }
    ...
    String st = "json test";
    if(mqttmessage == "start_continuous")
    {
        sendOne = false;
        Freeze = false;
    }
...
void loop() {
    ...
    //mqtt.publishMessage("scan", message);
    delay(1000);
    Serial.print(sendOne);
    if(sendOne){
